This is a sample ASP server-side library for the CoolCalc MJ8 application.  We are not a ASP shop, do not use this code "as is" but evaluate if changes are required to meet your production criteria.  Suggestions for improvement and pull requests are welcome.
...
10. The local JavaScript file fun-factory.js (coolcalc/javascript/mj8/v-bootstrap/staging/) contains handler functions for PDF rendering and email send functionality.  These functions are injected to the CoolCalc JavaScript library when the user interface is initialized.  You can edit the local functions any way you see fit as long as you do not change the function signatures.  In their standard implementation the PDF and email handlers make a POST request to a local endpoint on your server with a given HTML URL (the MJ8 report the user wants to download or email) and email in the payload.  Your local endpoint is responsible for generating the PDF or sending the email.  These endpoints are not implemented in this sample library as most clients will different APIs or third party libraries for this functionality (for information only, the coolcalc.com site uses api2pdf.com for PDF generation)

11. If you want to use api2pdf.com there is a sample in (coolcalc/client/staging/api2pdf.ashx), you will only need add your api_key in line 34.
